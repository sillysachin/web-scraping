...
public class ScrapeMCIv1 {
	static Logger LOGGER = Logger.getLogger(ScrapeMCIv1.class.getName());
	...
	public static void main(String[] args) {
		...
		int numberOfRaids = 0;
		if (args.length > 1) {
			numberOfRaids = Integer.valueOf(args[1]);
		} else {
			numberOfRaids = TOTAL_NUMBER_OF_DOCTORS / REQUEST_PER_HOUR;
		}

		int numberOfRequests = 0;
		if (args.length > 2) {
			numberOfRequests = Integer.valueOf(args[2]);
		} else {
			numberOfRequests = REQUEST_PER_HOUR;
		}
		...
		try {
			...
			scrapeOnline(numberOfRaids, numberOfRequests, sleep, bufferedWriter);
		} catch (IOException e) {
	...
	private static void scrapeOnline(int numberOfRaids, int numberOfRequests,
			int sleep, BufferedWriter bufferedWriter) throws IOException {
		next = 1;
		for (int i = 1; i <= numberOfRaids; i++) {
			end = i * numberOfRequests;
			LOGGER.info("Next - " + next + " , End - " + end);
			for (int j = next; j <= end; j++) {
				LOGGER.info("FindDoctor - " + j);
				String doctorInfo = findDoctor(j);
				bufferedWriter.write(doctorInfo
						+ System.getProperty("line.separator"));
				doctors.add(doctorInfo);
			}
			next = end + 1;
			LOGGER.info("SleepCounter - " + (i + 1));
			try {
				Thread.sleep(sleep);
