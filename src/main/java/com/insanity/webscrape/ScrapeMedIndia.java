...
public class ScrapeMedIndia {
	private static final int NUMBER_OF_PAGES = 1;
	static Logger LOGGER = Logger.getLogger(ScrapeMedIndia.class.getName());
	...
	private static void scrapeOnline(int justDialSearchParam) {
		Document doc = null;
		try {
			String medIndiaSearch = "http://www.medindia.net/patients/hospital_search/hospitals-bangalore.htm";
			Document searchDoc = Jsoup.connect(medIndiaSearch).get();
//			Elements selects = searchDoc.select("a[class=comp_name_text]");
//			for (Element element : selects) {
//				String href = medIndiaSearch + element.attr("href");
